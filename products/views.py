...
import pandas as pd
from .utils import get_simple_plot, get_salesperson_from_id, get_image
...
def sales_dist_view(request):
	...
	df['salesperson_id'] = df['salesperson_id'].apply(get_salesperson_from_id)
	df.rename({'salesperson_id':'salesman'}, axis=1, inplace=True)
